<?php 
    ...
    if ( isset($_POST['btn-add_equipe']) ) {
		...
		$extension = strrchr($_FILES['CollegueImage']['name'], '.');
		if(!in_array($extension, $extensions))
		{
		     $errMSG_add_equipe = 'Vous devez uploader un fichier de type png, gif, jpg, ou jpeg.';
		} else {
			if(move_uploaded_file($_FILES['CollegueImage']['tmp_name'], $dossier.$image)) {
				$errMSG_upload = "Le fichier a bien été sauvegardé.";
			} else {
				$errMSG_upload = "Impossible de copier le fichier dans ".$dossier;
		        exit();
			}
	    	$imgPathMembre = $dossier.$image;
        
	        $nomMembre = htmlspecialchars($_POST['CollegueNom']);
	        $prenomMembre = htmlspecialchars($_POST['CollegueImgSource']);
	        ...
	        if ($res) {
	            $errTyp = "Succès";
	            $errMSG_add_equipe = $errMSG_upload." Vous avez ajouté un nouvau membre à votre équipe.";
	            unset($nomMembre);
	            unset($prenomMembre);
	            unset($imgPathMembre);
	            ...
	            $errTyp = "danger";
	            $errMSG_add_equipe = $errMSG_upload." Quelque chose a mal fonctionné, réessayez plus tard ...";
	        }
    	}
    }
?>
	...
	<div class="back_left_content">
		...
		<h2>Ajouter un collègue</h2>
			<form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" autocomplete="off" enctype="multipart/form-data">
				...
				<input type="file" name="CollegueImage">
